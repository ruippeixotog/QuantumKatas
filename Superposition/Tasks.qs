...
namespace Quantum.Kata.Superposition {
    ...
    operation WState_PowerOfTwo (qs : Qubit[]) : Unit {
        if(Length(qs) == 1) {
            ...
            let half = Length(qs) / 2;
            WState_PowerOfTwo(qs[0..half-1]);

            using (anc = Qubit[1]) {
                H(anc[0]);
                for(i in 0..half-1) {
                    (Controlled SWAP)(anc, (qs[i], qs[half+i]));
                }
                for(i in 0..half-1) {
                    CNOT(qs[half+i], anc[0]);
                }
            }
        }
    }
    ...
    operation WState_Arbitrary (qs : Qubit[]) : Unit is Ctl {
        ...
        if(N == 1) {
            ...
            Ry(2.0 * ArcCos(Sqrt(1.0 / IntAsDouble(N))), head);
